...
def blend_faces(src_img, dst_img):
    PREDICTOR_PATH = 'models/blending/landmark_model.dat'
    predictor = dlib.shape_predictor(PREDICTOR_PATH)
    src_img_cv2 = np.array(src_img)
    src_img_cv2 = src_img_cv2[:, :, : :-1].copy()
    dst_img_cv2 = np.array(dst_img)
    dst_img_cv2 = dst_img_cv2[:, :, : :-1].copy()

    #src_img = cv2.imread('Unknown-4.jpg')
    #dst_img = cv2.imread('Unknown-6.jpg')

    # Select src face
    src_points, src_shape, src_face = select_face(predictor, src_img_cv2)
    # Select dst face
